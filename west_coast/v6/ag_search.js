...
window.addEventListener("load", () => {
  watchElements(".dn-search-field-container input", inputEl => {
    ...
    function renderFacets(facets) {
      ...
      if (!facets) {
        console.warn("No facets returned from Algolia");
        facetsPane.appendChild(applyBtn);
        return;
      }
      ...
      // Try different possible attribute name variations
      const attrVariations = {
        brand: ["brand", "Brand", "BRAND"],
        colors: ["colors", "color", "Colors", "Color", "COLOURS", "colours"],
      ...
      attrs.forEach((attr) => {
        ...
        // Try to find the facet data with different name variations
        for (const variation of attrVariations[attr] || [attr]) {
          if (facets[variation]) {
            counts = facets[variation];
            console.log(`Found facet "${attr}" as "${variation}":`, counts);
            break;
          }
        }
        
        // If still not found, try the original attribute name
        if (Object.keys(counts).length === 0 && facets[attr]) {
          counts = facets[attr];
        }
        ...
        if (Object.keys(counts).length === 0) {
          ...
          emptyLi.textContent = "No options available";
    ...
    function performSearch(q) {
      ...
      index
        .search(q, {
          ...
          facets: ["brand", "colors", "sizes", "color", "size"], // Try multiple variations
          maxValuesPerFacet: 20
        })
        .then((response) => {
          console.log("Full Algolia response:", response);
          const { hits, nbHits, facets, facetStats } = response;
          
          // Handle different response formats
          const facetsData = facets || response.facets || {};
    ...
    const searchContainer = document.querySelector(".dn-search-field-container");
    const input = document.getElementById("search");
    const icon = document.querySelector(".dn-search-icon");
    const button = document.querySelector(".voice-search-btn");

    if (searchContainer && input && icon && button) {
      icon.style.order = "1";
      input.style.order = "2";
